(function (){
	app.controller('ProductCtrl', function($scope, $timeout, $mdSidenav, 
	    ...
	    function loadData(){	   	
			$scope.categoriesTmp = CoResource.resources.Item.list({ type: 'category', 'parent_name': 'product', 'ignore-offset': 1}, function (){
	    ...
	    $scope.changeOffset = function (offset){
	 	...
	 	$scope.createPage = function(ev) {
    		$mdDialog.show({
				...
				templateUrl: '/templates/customs.new-product',
  		...
  		$scope.editCategory = function (item, ev){
  			$mdDialog.show({
				...
				targetEvent: ev,
				locals: {
					$current: _.extend(item, {type_name: 'product'}),
					$types: $scope.types
				}
    		})
    		.then(function(answer) {
