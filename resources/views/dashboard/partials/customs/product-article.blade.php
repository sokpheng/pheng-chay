        ...
        <h2>
	        <span ng-if="mode!=='locale'">			
			    <span ng-if="mode=='create'">
			    	Create <b><% product.display_name || '' %></b> article of <b><% category.display_name || (type.display_name || '') %></b>
			    </span>
			    <span ng-if="mode=='edit'">
			    	Edit <b><% product.display_name || '' %></b> article of <b><% category.display_name || (type.display_name || '') %></b>
			    </span>
			</span>

          	<span ng-if="mode=='locale'">
          		Edit your new article's locale
		...
		<form layout-padding="16" name="createPostForm">
	      	...
	      	<div layout layout-sm="column"> 
				...
				<md-input-container flex>
	      		...
	      		<md-input-container flex>
			        <md-select placeholder="Category" ng-model="data.category_id" 
			        	ng-disabled="type.name !== 'solution'"
	      	...
	      	<div layout="column" 
	      		ng-if="['video', 'grid-image', 'cover-image'].indexOf(category.name) == -1">
	        ...
	        <div class="custom-field"
	      	...
	      	<div layout="column" 
	      		ng-if="['grid-image', 'video', 'cover-image'].indexOf(category.name) == -1">
				...
				<div class="chips-group" flex ng-if="type.name == 'solution'">
					<md-chips ng-model="select_products" 
						ng-change="itemChangeProducts()"
						md-autocomplete-snap 
						md-require-match="true">
						<md-autocomplete
					          md-selected-item="selectedItem"
					          md-search-text="searchText"
					          md-items="item in querySearchProducts(searchText)"
					          md-search-text-change="itemChangeProducts()"
					          md-item-text="item.name"
	      	...
	      	<div ng-if="parentName == 'ArticleCtrl'" 
		      	...
		      	<section layout="row" layout-sm="column" 
		      		...
		      		<div class="media" flex="33">
		      			<div class="logo-inner" layout="column" layout-align="center center">
			         		...
			         		<span class="upload-text"
	      						ng-if="['video'].indexOf(category.name) == -1">Upload photo here
	      					</span>
			         		<span class="upload-text"
	      						ng-if="['video'].indexOf(category.name) != -1">Add YouTube link here
	      					</span>
			         	...
			         	<span class="icon-upload3 center"
	      					ng-if="['video'].indexOf(category.name) == -1"
			         		ngf-select ng-model="files" ngf-change="uploadMedia(files)" 
			         		multiple="false">
			         	</span>
			         	<span class="icon-youtube action center" 
	      					ng-if="['video'].indexOf(category.name) != -1"
		      		...
		      		<div class="media uploaded" flex="33" ng-repeat="media in data.photos"
			         	...
			         	<div class="logo-inner" layout="column" 
		      				layout-align="center center"
		      				ng-if="media.storage_type !== 'local'"
		      				style="background-image: url('<% media.file_name ? 'https://img.youtube.com/vi/' + media.file_name + '/0.jpg' : '' %>')">
			         		<div class="overlay"></div>
	        ...
	        <section layout="row" layout-sm="column" 
	        	...
	        	<section layout="row" layout-sm="column" layout-align="start center" flex="50">
	        		<md-button class="md-raised" 
		      		...
		      		<md-button class="md-raised" 
	        		    ng-if="data.status !== 'draft' && mode == 'edit'"
